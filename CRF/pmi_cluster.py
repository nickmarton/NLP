...
def document_generator2(path, lower=False):
    """."""
    with open(path, 'rb') as f:
        for line in f:
            triplets = line.strip().split("    ")
            tuples = [triplet.split("  ") for triplet in triplets]
            yield tuples
...
def make_word_counts(old, document_generator, max_vocab_size=None, min_word_count=1):
    res = defaultdict(int)
    ...
    if old:
        ...
        res_copy = defaultdict(int)
        for k,v in res.iteritems():
            res_copy[k[0]] += v

        words = sorted(res_copy.keys(), key=lambda w: res_copy[w], reverse=True)
        ...
        # only keep words that occur more frequently than too_rare
        words = [w for w in words if res_copy[w] > too_rare]
...
class DocumentLevelClusters(object):
    ...
    to consider.
    '''
    def __init__(self, old, doc_generator, word_counts, batch_size=1000):
        self.old = old
        ...
        for doc in doc_generator:
            for w in doc:
                ...
                if self.old:
                    ...
                    if tuple(w) not in self.word_counts:
                        continue
                    if doc_id not in self.index[tuple(w)]:
                        self.index[tuple(w)][doc_id] = 0
                    self.index[tuple(w)][doc_id] += 1
            doc_id += 1
        #'''
...
def main():
    ...
    if old:
        ...
        doc_gen = document_generator2

    word_counts = make_word_counts(old, doc_gen(args.input_path, lower=args.lower),
                                   max_vocab_size=args.max_vocab_size,
                                   min_word_count=args.min_word_count)

    c = DocumentLevelClusters(old, doc_gen(args.input_path, lower=args.lower),
